...
from schemas import UserCreate, UserAddressCreate, ChangePassword, UserUpdate
...
def update_user_password(db: Session, password_data: ChangePassword) -> User:
    try:
        user = get_user_by_email(db, email=password_data.email)
        if not user:
            raise HTTPException(
...
def update_user_profile(db: Session, email: str, user_data: UserUpdate) -> User:
    try:
        user = get_user_by_email(db, email)
        if not user:
            raise HTTPException(
        ...
        if user_data.email and user_data.email != email:
            ...
            if existing_user:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Новый адрес электронной почты уже зарегистрирован"
                )
...
def create_user_address(db: Session, address: UserAddressCreate, user_id: int) -> User_address:
    try:
...
def get_user_addresses(db: Session, user_id: int) -> List[User_address]:
    try:
        return db.query(User_address).filter(User_address.user_id == user_id).all()
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
